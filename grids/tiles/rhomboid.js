...
class RhomboidalGrid extends BaseGrid {
    ...
    getFragmentShaderSource(isWebGL2 = false) {
        if (isWebGL2) {
            ...
            // Function to get different shades based on rhombus type
            vec4 getRhombusColor(vec4 baseColor, int rhombusType) {
                if (rhombusType == 0) {
            ...
            void main() {
                ...
                if (cellValue.a > 0.5) {
                    // Apply different shade based on rhombus type
                    outColor = getRhombusColor(uDrawColor, rhombusType);
                } else {
                    outColor = uBgColor;
                }
            }`;
            ...
            vec4 getRhombusColor(vec4 baseColor, float rhombusType) {
                if (rhombusType == 0.0) {
                    return baseColor;
            ...
            void main() {
                ...
                if (cellValue.a > 0.5) {
                    gl_FragColor = getRhombusColor(uDrawColor, rhombusType);
                } else {
                    gl_FragColor = uBgColor;
                }
            }`;
    ...
    drawCanvasCells(ctx, cells) {
        ...
        for (const [key, state] of cells) {
            if (state) {
                ...
                this.drawAdjacentRhombus(ctx, centerX, centerY, radius, rhombusType);
            }
        }
    }

    drawAdjacentRhombus(ctx, centerX, centerY, radius, rhombusType) {
        // Calculate different shades based on rhombus type
        let shadeMultiplier = 1.0;
        if (rhombusType === 1) {
            shadeMultiplier = 0.75;
        } else if (rhombusType === 2) {
            shadeMultiplier = 0.5;
        }

        // Get current fill style and apply shade
        const originalFillStyle = ctx.fillStyle;
        if (typeof originalFillStyle === 'string' && originalFillStyle.startsWith('#')) {
            // Parse hex color and apply shade
            const hex = originalFillStyle.replace('#', '');
            const r = parseInt(hex.substr(0, 2), 16);
            const g = parseInt(hex.substr(2, 2), 16);
            const b = parseInt(hex.substr(4, 2), 16);
            ctx.fillStyle = `rgb(${Math.round(r * shadeMultiplier)}, ${Math.round(g * shadeMultiplier)}, ${Math.round(b * shadeMultiplier)})`;
        } else if (typeof originalFillStyle === 'string' && originalFillStyle.startsWith('rgb')) {
            // Parse rgb color and apply shade
            const rgbValues = originalFillStyle.match(/\d+/g);
    ...
    initGridTexture(gl, gridCols, gridRows) {
        ...
        // Use 3x width to store three rhombuses per hex
        const textureWidth = gridCols * 3;
        const textureHeight = gridRows;
        this.textureData = new Uint8Array(textureWidth * textureHeight * 4);

        // Initialize texture with all rhombuses empty
    ...
    resizeGridTexture(gl, newCols, newRows, oldCells) {
        ...
        if (oldCells) {
            for (const [key, state] of oldCells) {
                const [q, r, rhombusType] = key.split(',').map(Number);
                this.setCellState(gl, q, r, rhombusType, state);
            }
        }
    }
